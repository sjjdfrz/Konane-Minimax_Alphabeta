...
class KonaneGame:
    ...
    def evaluate(self, board, color, terminal_value=0):
        ...
        value += 50 * corner.count(color)
        value -= 50 * corner.count(self.opponent(color))
        ...
        value += 30 * edge.count(color)
        value -= 30 * edge.count(self.opponent(color))
        ...
        value += 20 * center.count(color)
        value -= 20 * center.count(self.opponent(color))

        # تعداد مهره ها
        value += 15 * board.count_symbol(color)
        value -= 15 * board.count_symbol(self.opponent(color))
